...
import SVG from "./svg";
import { guardarIncidencias } from "../store/actions/incidenciaActions";

const windowWidth = Dimensions.get("window").width;
const windowHeight = Dimensions.get("window").height;
import Buscar from "../assets/Buscar.svg";

export default function HeaderFilterType({
  filtronombre,
  navigation,
  filtrar,
}) {
  ...
  const filtrosOriginals = useReduxSelector(
    (state) => state.user.filtrosOriginals
  );
    ...
    marcados.forEach((element, index) => {
      if (element === true) {
        ...
        incidenciasOriginals.forEach((inc, index1) => {
          if (
            filtronombre === "tipo" &&
            inc.tipo_incidencia === filtrosOriginals[index].nombre
          ) {
            filtradasIncidencias.push(incidenciasOriginals[index1]);
          }
          if (
            filtronombre === "estado" &&
            inc.estado === filtrosOriginals[index].nombre
          ) {
            filtradasIncidencias.push(incidenciasOriginals[index1]);
          }
          if (
            filtronombre === "interaccion" &&
            inc.tipo_participacion === filtrosOriginals[index].nombre
          ) {
            filtradasIncidencias.push(incidenciasOriginals[index1]);
          }
        });
